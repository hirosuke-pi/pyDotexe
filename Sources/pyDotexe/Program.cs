    ...
    class Program
    {
        ...
        {           
            ...
            for (int i = 0; i < args.Length; i++)
            {
                try
                {
                    ...
                    else Console.WriteLine("[!] コマンドが見つかりません: " + args[i]);
                    }
                catch (Exception) { }
            }        
            bset.Decision(); // Get python version and get any paths.
            Console.Title = "pyDotexe-JP "+ version +" - Build: "+ bset.source_path;
            ...
            // Build successful.
            Console.WriteLine("\a\r\n[+] ビルドに成功しました。 (Python "+ bset.py_version_raw +".x)");
            Console.WriteLine("[*] " + bset.output_path);
            if (bset.copy_error.Count > 0)
            {
                Console.WriteLine("\r\n[-] いくつかのファイルはコピーできませんでした:");
        ...
        static void Main_short_help()
        {
            Console.WriteLine("\r\n" +
                ...
                "         (-pyver [PYTHON_VERSION(int)]) (--part-enc) (--zip-out) (--no-cache)\r\n" +
                ...
                "         -version (Show pyDotexe version    : " + version + "_JP)\r\n" +
