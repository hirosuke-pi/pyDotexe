...
namespace pyDotexe.Builder
{
    class Analysis
    {
        ...
        public static bool Start(ref BuildSet BuildSettings)
        {
            ...
            {
                bset = BuildSettings;
                Console.WriteLine("[+] 基本モジュールのファイルパスを取得しています...");
            ...
            {
                Console.WriteLine("[-] 選択されたソースコードの読み込みに失敗しました。\r\n[!] " + ex.Message);
            }
            catch (MakeModuleLoaderError ex)
            {
                Console.WriteLine("[-] ファイル 'module_load_pyDotexe.py' を作成できませんでした。\r\n[!] " + ex.Message);
            }
            catch (GetImportFilePathError)
            {
                Console.WriteLine("[-] インポートエラーを検出しました。ソースコードを確認してください。");
            }
            catch (GetModulesByProcessError ex)
            {
                Console.WriteLine("[-] Pythonのフォルダパスを取得できませんでした。\r\n[!] " + ex.Message);
            }
            finally
            {
                try
