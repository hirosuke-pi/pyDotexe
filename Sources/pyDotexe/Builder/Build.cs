    ...
    class Build
    {
        ...
        public static bool Start(ref BuildSet BuildSettings)
        {
            try
            {
                ...
                {
                    Console.WriteLine("\r\n[+] Pythonの基本ライブラリ・バイナリファイルをコピーしています...");
                ...
                {
                    Console.WriteLine("[+] リソースデータをコピーしています...");
                ...
                {
                    Console.WriteLine("\a\r\n[+] デバッグ用のコマンドを出力しました。コマンドプロンプトで実行してください。");
                ...
                if (bset.optimize)
                {
                    Console.WriteLine("[+] ソースコードを最適化しています...");
                    optimize_code_start();
                }
                Console.WriteLine("[+] コピーされたモジュールをコンパイルしています...");
                replace_compile_file(); // Replace default source codes file to compiled file.

                if (bset.one_file)
                {
                    Console.WriteLine("[+] イメージデータを作成しています...");
                    compress_modules(); // Compress TMP folder.
                }
                else
                {
                    Console.WriteLine("[+] 一時ファイルをコピーしています...");
            ...
            {
                Console.WriteLine("[-] 一時ファイルを作成できませんでした。");
            }
            catch (ImportDllCompilerError)
            {
                Console.WriteLine("\r\n[-] いくつかのファイルはコピーされませんでした: python.exe, pythonw.exe, python" + bset.py_version + ".dll, ソースコード。");
            }
            catch (ReplaceCompileFileError)
            {
                Console.WriteLine("[-] ソースコードのコンパイルに失敗しました。");
            }
            catch (CompressModulesError)
            {
                Console.WriteLine("[-] イメージファイルを生成できませんでした。");
            }
            return false;
        }
        ...
        private static void copy_module_file()
        {
            ...
            {
                Console.WriteLine("[-] Pythonのバイナリファイルのアイコン変更に失敗しました。r\n[!] " + ex.Message + "\r\n");
            }
        }
        ...
        private static void add_fixes_argv_path()
        {
            ...
            {
                Console.WriteLine("[!] 固定パラメータを設定できませんでした: "+ ex.Message +"\r\n");
            }
        }
        ...
        private static void compress_zip()
        {
            ...
            foreach (string file in files)
            {
                ...
                {
                    Console.WriteLine(" - エラー: "+ file +" ["+ ex.Message+"]");
                    continue;
                }
            }
            // Close objects.
